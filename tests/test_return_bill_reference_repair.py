...
class ReturnBillReferenceRepairTests(unittest.TestCase):
    ...
    def test_light_place_return_targets_tally_bill_216(self):
        light = next(row for row in REPAIRS if row["return"] == "PRET-26-00002")
        self.assertEqual(
            [(row["target"], row["amount"]) for row in light["allocations"]],
            [("PINV-26-00057", "16231.00")],
        )
